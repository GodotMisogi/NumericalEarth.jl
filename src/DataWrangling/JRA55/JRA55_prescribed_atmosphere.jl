...
With `prefetch = true` each variable's next sliding window is loaded asynchronously on a background
thread so the reload spike is hidden behind compute (requires `JULIA_NUM_THREADS ≥ 2`).
